    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbar-menu">
            <ul class="nav navbar-nav header-menu-zone" data-in="fadeInDown" data-out="fadeOutUp">
                ...
                <style>
                    /* show some texte when it is not a pc */
                    @media only screen and (max-width: 992px) {
                        .for-phone {
            ...
            <ul class="nav navbar-nav" data-in="fadeInDown" data-out="fadeOutUp" style="float: right;">
                ...
                <li class="btn-deposer-annonce-li for-pc" style="padding-right: 0px !important">
                    @if (auth()->check() && (auth()->user()->hasRole('Professionnel') || auth()->user()->hasRole('Administrateur')))
                        <a class="add-annonce btn-deposer-annonce" id="" href="{{ route('public.annonces.create') }}">
                            Déposer une annonce
                        </a>
                    @elseif (auth()->check() && auth()->user()->hasRole('Usager'))
                        <a class="add-annonce btn-deposer-annonce" id="" href="{{ route('pricing') }}">
                            Déposer une annonce
                        </a>
                    @else
                        <a class="add-annonce btn-deposer-annonce" id="" data-toggle="modal" data-target="#signin" href="javascript:void(0)" onclick="$('#share').hide()">
                            Déposer une annonce
                        </a>
                    @endif
                </li>
                ...
                <li class="for-phone">
                    @if (auth()->check() && (auth()->user()->hasRole('Professionnel') || auth()->user()->hasRole('Administrateur')))
                        <a href="{{ route('public.annonces.create') }}">
                    ...
                    @else
                        <a data-toggle="modal" data-target="#signin" href="javascript:void(0)" onclick="$('#share').hide()">
                            <i class="fa fa-plus" aria-hidden="true"></i> &nbsp;
                            Déposer une annonce
                        </a>
                    @endif
                ...
                {{-- <li class="btn-deposer-annonce-li" style="padding-right: 0px !important">
                    <a class="add-annonce" id="btn-deposer-annonce" href="{{ route('public.annonces.create') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="35" height="35"><!--!Font Awesome Free 6.7.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
                ...
                @if (auth()->check())
                    <li class="btn-deposer-annonce-li no-pd dropdown for-pc">
