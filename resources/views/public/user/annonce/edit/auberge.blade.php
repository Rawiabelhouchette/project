...
@section('title', 'Modifier une Auberge')
...
@section('content')
    ...
    <x-breadcumb backgroundImage="{{ asset('assets_client/img/banner/image-2.jpg') }}" :showTitle="true"
        title="Modifier une Auberge" :breadcrumbs="$breadcrumbs" />

    <div class="page-name auberge row">
        <div class="container text-left p-0">
            @livewire('admin.auberge.edit', ['auberge' => $auberge])
        </div>
    </div>
@endsection
...
@section('js')
    ...
    <script>
        ...
        var latlng = L.latLng({{ $auberge->annonce->latitude }}, {{ $auberge->annonce->longitude }});
